  ...
  if (isMain) { //
    ...
    win.webContents.on('before-input-event', (event, input) => {
     if (input.control && input.shift && input.key.toLowerCase() === 'r') {
        console.log('[ATAJO] Ctrl+Shift+R detectado - Volviendo al selector (FORZADO)');
        event.preventDefault();
        
        // 🚨 FIX CLAVE: Eliminar el listener de 'close' para evitar la interferencia del Cierre Seguro.
...
// JavaScript/windowManager.js (Mejora)
function closeAllWindows() { //
    // Cierra el selector si existe
    if (appState.winSelector && !appState.winSelector.isDestroyed()) { appState.winSelector.close(); } //
    appState.winSelector = null; //

    // Cierra las ventanas principales
    appState.windows.forEach(w => { //
        if (w && !w.isDestroyed()) { w.close(); } //
    });
    appState.windows = []; // Vacia el array inmediatamente

    // Cierra las ventanas de fondo
    appState.backgroundWindows.forEach(w => { //
        if (w && !w.isDestroyed()) { w.close(); } //
    });
    appState.backgroundWindows = []; // Vacia el array inmediatamente
}
